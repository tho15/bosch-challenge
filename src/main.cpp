...
int get_closing_car_behind(const vector<Car> & cars, double car_d, double car_s, double check_dist)
{
	int nc = cars.size();
	double td;
...
)
{
	...
	for(int i = num_prev; i > 0; --i) {
		map_idx.push_back(idx-i);
	}
	
	for(int i = 0; i < num_next; ++i) {
		if(idx+i > map_s.size()-1) break;
		map_idx.push_back(idx+i);
	}
...
vector<Car> get_cars_in_lane_segment(const vector<Car> &cars, double car_s, double car_d, double fdist, double bdist)
{
	...
	for(int i = 0; i < cars.size(); ++i) {
		...
		if(cars[i].car_s > s1 && cars[i].car_s < s2) {
...
)
{
	...
	cout << "calculated lane cost: " << fl_cost << " " << lcl_cost << " " << lcr_cost << endl;
	
	if(lcl_cost <= lcr_cost && lcl_cost < fl_cost) {
   		return _CS_LCL;
	}
	else if(lcr_cost < lcl_cost && lcr_cost < fl_cost) {
   		return _CS_LCR;
   	}
   	else {
    	return _CS_KL_FL;
